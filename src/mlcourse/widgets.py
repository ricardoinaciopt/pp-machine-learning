"""Interactive laboratories with visible controls and testable renderers."""
...
def interactive_knn(X_train, X_test, y_train, y_test):
    ...
    controls = {
        "n_neighbors": control(
            ...
            value=5,
            min=1,
            max=min(31, len(X_train)),
            step=2,
        ),
        "weights": control(
            widgets.Dropdown, "weights", options=["uniform", "distance"]
        ),
    }
    return classification_lab(
        lambda **p: make_pipeline(StandardScaler(), KNeighborsClassifier(**p)),
        controls,
        X_train,
        X_test,
        y_train,
...
def interactive_tree(X_train, X_test, y_train, y_test):
    ...
    controls = {
        "max_depth": control(widgets.IntSlider, "max_depth", value=3, min=1, max=12),
        "min_samples_leaf": control(
            widgets.IntSlider, "min_samples_leaf", value=1, min=1, max=20
        ),
    }
    return classification_lab(
        lambda **p: DecisionTreeClassifier(**p, random_state=0),
        controls,
        X_train,
        X_test,
        y_train,
...
def interactive_svm(X_train, X_test, y_train, y_test):
    ...
    controls = {
        "kernel": control(
            widgets.Dropdown, "kernel", options=["linear", "poly", "rbf"], value="rbf"
        ),
        "C": control(
            widgets.FloatLogSlider, "C", value=1.0, base=10, min=-2, max=2, step=0.5
        ),
        "gamma": control(
            widgets.FloatLogSlider, "gamma", value=1.0, base=10, min=-2, max=1, step=0.5
        ),
    ...
    relevant()
    return classification_lab(
        lambda **p: make_pipeline(StandardScaler(), SVC(**p)),
        controls,
        X_train,
        X_test,
        y_train,
...
def interactive_mlp(X_train, X_test, y_train, y_test):
    ...
    controls = {
        "width": control(
            widgets.IntSlider, "Hidden-layer width", value=5, min=1, max=30
        ),
        "depth": control(
            widgets.IntSlider, "Hidden-layer count", value=1, min=1, max=3
        ),
        "activation": control(
            widgets.Dropdown,
            "activation",
            options=["tanh", "relu", "logistic", "identity"],
        ),
        "alpha": control(
            widgets.FloatLogSlider, "alpha", value=0.01, base=10, min=-4, max=1, step=1
        ),
        "random_state": control(
            widgets.IntSlider, "random_state", value=0, min=0, max=10
        ),
    }
...
def interactive_regression(X_train, X_test, y_train, y_test):
    ...
    controls = {
        "max_depth": control(widgets.IntSlider, "max_depth", value=3, min=1, max=12),
        "min_samples_leaf": control(
            widgets.IntSlider, "min_samples_leaf", value=5, min=1, max=40
        ),
    }

    def draw(max_depth, min_samples_leaf):
        models = {
            "Linear plane": LinearRegression(),
        ...
        for ax, (name, model), surface in zip(axes, models.items(), surfaces):
            picture = ax.pcolormesh(
                xx, yy, surface, cmap="viridis", shading="auto", vmin=lower, vmax=upper
            )
            ax.scatter(
                X_train.iloc[:, 0],
                X_train.iloc[:, 1],
...
def interactive_clustering(X, algorithm="K-Means"):
    ...
    controls = {
        ...
        "eps": control(
            widgets.FloatSlider, "eps", value=0.5, min=0.05, max=2.0, step=0.05
        ),
        "min_samples": control(
            widgets.IntSlider, "min_samples", value=5, min=2, max=20
        ),
        "linkage": control(
            widgets.Dropdown,
            "linkage",
    ...
    def draw(algorithm, n_clusters, eps, min_samples, linkage):
        model = (
            KMeans(n_clusters=n_clusters, n_init=10, random_state=0)
...
def interactive_threshold(X_train, X_test, y_train, y_test):
    ...
    controls = {
        ...
        "class_weight": control(
            widgets.Dropdown,
            "class_weight",
            options=[("Uniform", None), ("Balanced", "balanced")],
        ),
    }
    # Fit the two fixed models once; threshold movement never retrains.
    models = {
        weight: make_pipeline(
            StandardScaler(), LogisticRegression(class_weight=weight)
        ).fit(X_train, y_train)
    ...
    def draw(threshold, class_weight):
        ...
        ConfusionMatrixDisplay.from_predictions(
            y_test, pred, labels=[0, 1], ax=axes[1], colorbar=False
        )
...
def interactive_meta_performance(meta_data, predictions):
    ...
    controls = {
        "dataset": control(
            widgets.Dropdown,
            "Dataset",
            options=meta_data.dataset.tolist(),
            value=meta_data.dataset.iloc[0],
        ),
        "feature": control(
            widgets.Dropdown,
    ...
    image = widgets.Image(
        format="png", layout=widgets.Layout(width="100%", max_width="1200px")
    )
    ...
    def draw(dataset, feature, strategy):
        ...
        axes[0].set(
            ...
            title=f"{strategy} across datasets",
        )
        axes[0].annotate(
            dataset,
        ...
        ranking["predicted_rank"] = ranking.predicted_accuracy.rank(
            method="min", ascending=False
        ).astype(int)
        ranking["measured_rank"] = ranking.measured_accuracy.rank(
            method="min", ascending=False
        ).astype(int)
        ranking = ranking.sort_values("predicted_rank").reset_index(drop=True)
        table.value = ranking.to_html(
