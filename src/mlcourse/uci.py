...
CONCRETE_COLUMNS = ['cement', 'slag', 'fly_ash', 'water', 'superplasticizer',
...
def normalise(raw, spec):
    if digest(raw) != spec['source_sha256']:
        raise ValueError(f"UCI {spec['uci_id']}: source checksum changed; review before updating the manifest")
...
def validate_frame(frame, spec):
    if frame.columns.tolist() != spec['columns'] or frame.shape != (spec['rows'], len(spec['columns'])):
    ...
    if not np.isfinite(frame.to_numpy()).all():
        raise ValueError('Missing or infinite values are not present in the frozen UCI source')
...
def inspect(frame, spec):
    report = {
        'uci_id': spec['uci_id'], 'rows': len(frame), 'columns': frame.columns.tolist(),
        'missing_cells': int(frame.isna().sum().sum()),
        'duplicate_rows_beyond_first': int(frame.duplicated().sum()),
        'ranges': {c: {'min': float(frame[c].min()), 'max': float(frame[c].max())} for c in frame},
    }
    if spec['uci_id'] == 165:
        report.update(ingredient_groups=len(frame[CONCRETE_COLUMNS[:7]].drop_duplicates()),
                      repeated_predictor_rows_beyond_first=int(frame[CONCRETE_COLUMNS[:8]].duplicated().sum()))
    else:
        report['category_counts'] = {c: {str(k):int(v) for k,v in frame[c].value_counts().sort_index().items()}
...
def fetch_datasets(root=ROOT, output_dir=None, dataset_ids=None):
    ...
    for spec in sources(root):
        ...
        if digest(content) != spec['normalised_sha256']:
            raise ValueError('Normalised CSV digest differs; check parser/serialization version')
    ...
    for spec, content, report in prepared:
        ...
        with tempfile.NamedTemporaryFile(dir=path.parent, delete=False) as file:
            temp = Path(file.name);file.write(content)
        temp.replace(path)
        (path.parent/'inspection.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
...
def load_dataset(dataset_id, root=ROOT):
    ...
    for folder in ['data', '.local-data']:
        ...
        if path.exists():
            ...
            validate_frame(frame,spec)
            return frame
    raise FileNotFoundError(f"UCI {dataset_id} is not cached. Restore the supplied data folder, or run python scripts/prepare_data.py --dataset {'concrete' if dataset_id == 165 else 'wholesale'} --publish from the repository root.")
