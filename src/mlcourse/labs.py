...
def load_course_data(name):
    paths = {
        "hotel": "ho01/HotelReservations.csv",
        "iris": "ho03/Iris.csv",
        "toy": "ho07/toy3.csv",
        **{f"ds{i}": f"shared/ds{i}.csv" for i in range(1, 4)},
    }
    root = find_root()
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"Course data missing: {paths[name]}. Restore the repository data folder and rerun setup."
        )
...
def grid(X, size=120):
    values = np.asarray(X)
    pad = np.maximum(np.ptp(values, axis=0) * 0.08, 0.1)
    xx, yy = np.meshgrid(
        np.linspace(values[:, 0].min() - pad[0], values[:, 0].max() + pad[0], size),
...
def boundary(ax, model, X, y, title, threshold=None):
    ...
    prediction = (
        ...
        else (model.predict_proba(points)[:, 1] >= threshold).astype(int)
    )
    ...
    names = (
        list(X.columns) if isinstance(X, pd.DataFrame) else ["Feature 1", "Feature 2"]
    )
...
def histogram_grid(frame):
    ...
    fig, axes = plt.subplots(
        rows, 3, figsize=(13, 3 * rows), squeeze=False, layout="constrained"
    )
...
def cluster_summary(X, labels):
    ...
    value = (
        silhouette_score(np.asarray(X)[keep], labels[keep])
        if 1 < count < keep.sum()
        else np.nan
    )
...
def cluster_plot(ax, X, labels, title):
    ...
    names = (
        list(X.columns)
        if isinstance(X, pd.DataFrame)
        else ["Coordinate 1", "Coordinate 2"]
    )
    ax.set(xlabel=names[0], ylabel=names[1], title=title)
...
def load_task_results():
    ...
    if not path.is_file():
        raise FileNotFoundError(
            "Course data missing: ho06/results.zip. Restore the repository data folder and rerun setup."
        )
    with zipfile.ZipFile(path) as archive:
        names = sorted(
            (
                ...
                if n.startswith("results_ds") and n.endswith(".csv")
            ),
            key=lambda n: int(Path(n).stem.replace("results_ds", "")),
        )
        return pd.DataFrame(
            {Path(n).stem: pd.read_csv(archive.open(n)).mean() for n in names}
        ).T
...
def load_meta_dataset():
    ...
    names = {
        "Dataset": "dataset",
        "mf_n_instances": "n_instances",
        "mf_n_features_raw": "n_features",
        "mf_n_classes": "n_classes",
        "mf_imbalance_ratio": "class_imbalance",
        "mf_missing_rate": "missing_rate",
        "mf_numeric_ratio": "numeric_ratio",
        "mf_class_entropy": "class_entropy",
        "AccuracyDT": "tree_accuracy",
        "AccuracyDS": "stump_accuracy",
        "AccuracyMajority": "majority_accuracy",
        "AccuracyGap_DT_minus_DS": "accuracy_gap",
        "BestModelBinary": "tree_wins",
    }
