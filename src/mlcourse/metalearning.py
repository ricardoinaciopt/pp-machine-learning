"""HO9's frozen, small algorithm-selection experiment. Network is opt-in."""
...
RANDOM_STATE = 42
PROTOCOL = {
    "version": 1, "seed": RANDOM_STATE, "test_size": 0.30,
...
FEATURES = ["mf_n_instances", "mf_n_features_raw", "mf_n_features_encoded", "mf_n_classes",
            "mf_feature_instance_ratio", "mf_instance_feature_ratio", "mf_numeric_ratio",
            "mf_missing_rate", "mf_class_entropy", "mf_majority_class_prop",
            "mf_minority_class_prop", "mf_imbalance_ratio", "mf_mean_abs_feature_corr"]
...
def read_manifest(root=ROOT):
    manifest = pd.read_csv(Path(root) / "data/ho09/openml_datasets.csv", keep_default_na=False)
...
def compute_simple_metafeatures(X, y):
    ...
    return dict(zip(FEATURES, [
        n, p, encoded.shape[1], len(probs), p/n, n/p,
        len(X.select_dtypes(include="number").columns)/p,
        float(X.isna().to_numpy().mean()), float(-(probs*np.log2(probs)).sum()),
        float(probs.max()), float(probs.min()), float(probs.max()/probs.min()),
...
def load_raw(entry, raw_dir, refresh=False):
    ...
    if refresh or not path.exists():
        ...
        for key, expected in [("id", entry.openml_id), ("name", entry.name),
                              ("version", entry.version), ("default_target_attribute", entry.target)]:
        ...
        if hashlib.sha256(data).hexdigest() != entry.sha256:
            raise ValueError(f"OpenML {entry.openml_id}: source checksum changed; review before refreezing")
        _atomic_write(path, data)
    if sha256(path) != entry.sha256:
        raise ValueError(f"Raw cache checksum mismatch: {path}")
    frame = pd.read_parquet(path)
    if frame.shape != (entry.n_instances, entry.n_features + 1) or entry.target not in frame:
...
def summarise_one_dataset(entry, X, y):
    train, test = train_test_split(np.arange(len(y)), test_size=PROTOCOL["test_size"],
                                  random_state=RANDOM_STATE, stratify=y)
    X_train, X_test, y_train, y_test = X.iloc[train], X.iloc[test], y.iloc[train], y.iloc[test]
    scores, correct, balanced = {}, {}, {}
    for label, depth in [("DT", None), ("DS", 1)]:
        model = Pipeline([("prepare", preprocessing(X_train)),
    ...
    return {
        "OpenMLID": entry.openml_id, "Dataset": entry.name, "Source": "OpenML",
        "NInstances": len(y), "NTrain": len(train), "NTest": len(test),
        "TrainIndexSHA256": hashlib.sha256(np.asarray(train, dtype="<i8").tobytes()).hexdigest(),
        "TestIndexSHA256": hashlib.sha256(np.asarray(test, dtype="<i8").tobytes()).hexdigest(),
        "CorrectDT": correct["DT"], "CorrectDS": correct["DS"],
        "AccuracyDT": scores["DT"], "AccuracyDS": scores["DS"],
        "BalancedAccuracyDT": balanced["DT"], "BalancedAccuracyDS": balanced["DS"],
        "AccuracyMajority": float((y_test == majority).mean()),
        "AccuracyGap_DT_minus_DS": difference/len(test),
        "Best": -int(np.sign(difference)),
        "BestModel": "Decision tree" if difference > 0 else "Decision stump" if difference < 0 else "Tie",
...
def rebuild_meta_dataset(root=ROOT, raw_dir=None, output_dir=None, refresh=False):
    ...
    csv_bytes = table.to_csv(index=False, float_format="%.12g", lineterminator="\n").encode()
    provenance = {
        "protocol": PROTOCOL,
    ...
    _atomic_write(destination / "meta_dataset.provenance.json",
...
def load_meta_dataset(root=ROOT):
    ...
    try:
        ...
        checks = {"manifest_sha256": folder / "openml_datasets.csv",
        ...
        if table.OpenMLID.tolist() != manifest.openml_id.tolist() or table.Dataset.tolist() != manifest.name.tolist():
            raise ValueError("HO9 cache does not match the frozen manifest")
        if not np.isfinite(table[FEATURES].to_numpy()).all():
            raise ValueError("Non-finite cached meta-features")
        return table
    except FileNotFoundError as error:
        raise FileNotFoundError("HO9 needs its committed cache. Restore data/ho09 or explicitly run python scripts/prepare_data.py --dataset meta-learning --publish") from error
...
def fit_meta_model(meta_data):
    ...
    predictions = pd.DataFrame({"Dataset": meta_data.Dataset.to_numpy(),
    ...
    model.fit(X, y)
    return model, predictions, accuracy_score(y, pred), confusion_matrix(y, pred, labels=[0, 1])
...
def correlation_table(meta_data, target_name="AccuracyGap_DT_minus_DS"):
    ...
    rows = [{"Meta-feature": feature, "Pearson correlation": meta_data[feature].corr(meta_data[target_name])}
            for feature in FEATURES if meta_data[feature].nunique() > 1 and meta_data[target_name].nunique() > 1]
    result = pd.DataFrame(rows, columns=["Meta-feature", "Pearson correlation"])
    result["abs(correlation)"] = result["Pearson correlation"].abs()
    return result.sort_values("abs(correlation)", ascending=False).reset_index(drop=True)
