"""Execute canonical notebooks in fresh kernels, without data downloads."""
import argparse
import tempfile
...
def run(path, output, simulate_colab=False, save_solutions=False):
    ...
    book.cells.insert(0, nbformat.v4.new_code_cell(PRELUDE + (COLAB if simulate_colab else '')))
        ...
        for model in bundle.get('state', {}).values():
            for result in model.get('state', {}).get('outputs', []):
                if result.get('output_type') == 'error':
                    raise RuntimeError(f"Widget callback failed in {path}: {result.get('evalue')}")
    book.cells.pop(0)
    ...
    if save_solutions and 'solutions' in path.parts:
        ...
        for cell in book.cells:
            if cell.cell_type == 'code':
                if 'interactive' in cell.metadata.get('tags', []) or cell.metadata.get('jupyter', {}).get('source_hidden'):
                    cell.outputs = []
...
def select_notebooks(paths=(), ho=None, root=ROOT):
    ...
    if paths:
        ...
        if any(path not in canonical for path in selected):
            raise ValueError('Each path must identify a canonical notebook in this repository')
    else:
        selected = [path for path in canonical if ho is None or f'ho{ho:02d}' in path.parts]
...
def main(argv=None):
    ...
    parser.add_argument('notebooks', nargs='*', type=Path, help='Specific notebook paths; defaults to all pairs.')
    parser.add_argument('--ho', type=int, choices=range(1, 10), help='Execute one student/solution pair.')
    parser.add_argument('--output-dir', type=Path, help='Keep executed copies here; otherwise use temporary files.')
    parser.add_argument('--simulate-colab', action='store_true', help='Exercise the Colab setup branch locally; this does not run hosted Colab.')
    parser.add_argument('--save-solutions', action='store_true', help='Refresh selected solutions with bounded static outputs.')
    ...
    if args.output_dir:
        ...
        if output == ROOT or ROOT / 'hands-on' == output or ROOT / 'hands-on' in output.parents:
            parser.error('Executed copies must be separate from the canonical notebooks')
    with tempfile.TemporaryDirectory(prefix='ac1-notebooks-') as directory:
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            jobs = [(path, pool.submit(run, path, output, args.simulate_colab, args.save_solutions)) for path in paths]
