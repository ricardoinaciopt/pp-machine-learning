...
class DatasetIntegrityTests(unittest.TestCase):
    ...
    def test_splits_are_deterministic_disjoint_and_exhaustive(self):
        ...
        self.assertEqual(
            sorted(np.concatenate([train, val, test]).tolist()), list(range(120))
        )
    ...
    def test_absent_cache_never_downloads(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root / "data").mkdir()
            shutil.copyfile(
                ROOT / "data/uci_sources.json", root / "data/uci_sources.json"
            )
            with patch(
...
class MetaDatasetTests(unittest.TestCase):
    ...
    def test_default_is_offline_and_ordered(self):
        ...
        self.assertEqual(
            table.OpenMLID.tolist(), ml.read_manifest(ROOT).openml_id.tolist()
        )
        self.assertEqual(
            set(ml.FEATURES), set((c for c in table if c.startswith("mf_")))
        )
    ...
    def test_test_rows_do_not_change_metafeatures_and_ties_are_exact(self):
        ...
        _, test = train_test_split(
            np.arange(60), test_size=0.3, random_state=42, stratify=y
        )
    ...
    def test_failed_build_does_not_replace_cache(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(
                ml, "load_raw", side_effect=RuntimeError("unavailable source")
            ):
    ...
    def test_readable_meta_data_preserves_cached_outcomes(self):
        ...
        np.testing.assert_allclose(
            shown.accuracy_gap, shown.tree_accuracy - shown.stump_accuracy
        )
...
class DataWorkflowTests(unittest.TestCase):
    def test_preparation_failure_preserves_all_published_data(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch.object(prepare_data, "fetch_datasets", side_effect=fetch),
                patch.object(
                    prepare_data,
    ...
    def test_preparation_uses_consistent_data_folders(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(
                prepare_data, "rebuild_meta_dataset", side_effect=rebuild
            ):
                prepare_data.prepare(
                    ["meta-learning"], destination, destination / "raw"
                )
            self.assertEqual(
                (destination / "ho09/meta_dataset.csv").read_text(), "candidate"
            )
        ...
        with (
            ...
            for destination in [ROOT / "data", ROOT / "data/ho09", ROOT]:
                with self.assertRaises(SystemExit):
                    prepare_data.main(
                        ["--dataset", "all", "--output-dir", str(destination)]
                    )
